...
struct AXAdBlockSettingsView: View {
    ...
    var body: some View {
        NavigationSplitView {
            List(
                FilterListCategory.allCases, id: \.self,
        ...
        .sheet(isPresented: $filterListManager.showingUpdatePopup) {
            WBUpdatePopupView(
                filterListManager: filterListManager,
                isPresented: $filterListManager.showingUpdatePopup)
        }
        ...
        .alert(
            ...
            isPresented: $filterListManager.showRecommendedFiltersAlert
        ) {
            Button("Enable") {
            ...
            Text(
                "No filters are currently enabled. Would you like to enable the recommended filters?"
            )
        }
        .alert(
            "Unapplied Changes",
            isPresented: $windowDelegate.shouldShowExitAlert
        ) {
            ...
            Text(
                "You have unapplied changes. Do you want to apply them before exiting?"
            )
        }
        .alert(
            "Reset to Default Lists?",
            isPresented: $filterListManager.showResetToDefaultAlert
        ) {
        ...
        .alert(
            "No Updates Found",
            isPresented: $filterListManager.showingNoUpdatesAlert
        ) {
            Button("OK", role: .cancel) {}
        }
        .onAppear {
            windowDelegate.hasUnappliedChanges = {
                filterListManager.hasUnappliedChanges
            }
...
struct FilterListContentView: View {
    ...
    private let cachedCategories = FilterListCategory.allCases.filter {
        $0 != .all
    }
    ...
    private var categoryContent: some View {
            ...
            ForEach(cachedCategories, id: \.self) { category in
                if !filterListManager.filterLists(for: category).isEmpty {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        ForEach(filterListManager.filterLists(for: category)) {
                            filter in
                            FilterRowView(
                                filter: filter,
                                filterListManager: filterListManager
        ...
        case .custom:
            ForEach(filterListManager.customFilterLists) { filter in
                FilterRowView(
                    filter: filter, filterListManager: filterListManager
                )
                .padding(.horizontal)
        ...
        default:
            ForEach(filterListManager.filterLists(for: selectedCategory)) {
                filter in
                FilterRowView(
                    filter: filter, filterListManager: filterListManager
                )
...
struct FilterStatsBanner: View {
    ...
    private var totalRulesCount: Int {
        if let containerURL = WBFileStorage.shared.cachedContainerURL,
            let data = try? Data(
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack(spacing: 20) {
                ...
                Button(action: {
                    if let url = URL(
                        string: filterURLString.trimmingCharacters(
                            in: .whitespacesAndNewlines))
                    {
                        filterListManager.addCustomFilterList(
                            name: filterName.trimmingCharacters(
                                in: .whitespacesAndNewlines),
                            url: url,
                            description: filterDescription.trimmingCharacters(
                                in: .whitespacesAndNewlines)
                        )
                        presentationMode.wrappedValue.dismiss()
                    } else {
